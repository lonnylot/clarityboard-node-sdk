...
function getRequestOpts(self, requestArgs, spec, overrideData) {
  ...
  for (var i = 0, l = urlParams.length; i < l; ++i) {
    ...
    if (param == 'id' && typeof arg !== 'string') {
      ...
      throw new Error(
        'Clarityboard: "id" must be a string, but got: ' + typeof arg +
    ...
    if (!arg) {
      ...
      path = self.createResourcePathWithSymbols(spec.path);
      throw new Error(
        'Clarityboard: Argument "' + urlParams[i] + '" required, but got: ' + arg +
        ' (on API request to `' + requestMethod + ' ' + path + '`)'
      );
    }
  ...
  if (args.length) {
    path = self.createResourcePathWithSymbols(spec.path);
    throw new Error(
      'Clarityboard: Unknown arguments (' + args + '). Did you mean to pass an options ' +
